...
// Claves de escritura: solo actualizan resultado, no el formulario
const TYPING_KEYS = ['cConc','cDil','cDeseada','vFinal','labelConc','labelDil'];

function setState(patch) {
  state = { ...state, ...patch };
  // Usuario escribiendo → solo actualiza resultado
  // Formulario intacto → teclado permanece abierto en el celular ✅
  const soloEscribiendo = Object.keys(patch).every(k => TYPING_KEYS.includes(k));
  if (soloEscribiendo && document.getElementById("result-zone")) {
    document.getElementById("result-zone").innerHTML = buildResultHTML();
  } else {
    render();
  }
}
...
function render() {
  ...
  app.innerHTML = `
      ...
      ${state.tipo==="Generica" ? `
        <div class="grid-2" style="margin-bottom:14px">
          <div class="input-wrap">
            ...
            <input class="text-input" value="${state.labelConc}" oninput="setState({labelConc:this.value})"/>
          </div>
          <div class="input-wrap">
            <span class="input-label">Nombre dil.</span>
            <input class="text-input" value="${state.labelDil}" oninput="setState({labelDil:this.value})"/>
          </div>
        </div>
      ` : ""}
      <div class="grid-2">
    ...
    <div id="result-zone">${buildResultHTML()}</div>
  `;
}
