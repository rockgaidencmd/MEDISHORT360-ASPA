/* ====================================================
   MEDISHORT360 - REGLA DEL ASPA (app.js)
   ==================================================== */

const LOGO_B64 = "data:image/jpeg;base64,";

const PRESETS = {
  "Salina": {
    concentrada: { label: "Soletrol Na 20%", value: 20, ampolla: { volMl: 10, label: "amp 10 mL" } },
    diluida: { label: "Sol. Salina 0.9%", value: 0.9 },
    opciones_conc: [
      { label: "Soletrol Na 20%", value: 20, ampolla: { volMl: 10, label: "amp 10 mL" } },
      { label: "NaCl 17.7%", value: 17.7, ampolla: { volMl: 10, label: "amp 10 mL" } },
    ],
    opciones_dil: [
      { label: "Sol. Salina 0.9%", value: 0.9 },
      { label: "Agua dest. 0%", value: 0 },
    ],
    deseada: 3,
    volumenes: [100, 250, 500, 1000],
    acento: "#c8a614",
    sombra: "#c8a61444",
    icon: "💧",
  },
  "Dextrosa": {
    concentrada: { label: "Dextrosa 50%", value: 50 },
    diluida: { label: "Dextrosa 5%", value: 5 },
    opciones_conc: [{ label: "Dextrosa 50%", value: 50 }],
    opciones_dil: [
      { label: "Dextrosa 5%", value: 5 },
      { label: "Agua dest. 0%", value: 0 },
    ],
    deseada: 20,
    volumenes: [100, 250, 500, 1000],
    acento: "#c41230",
    sombra: "#c4123044",
    icon: "🍬",
  },
  "Generica": {
    concentrada: { label: "Solucion concentrada", value: 50 },
    diluida: { label: "Solucion diluida", value: 0 },
    opciones_conc: [],
    opciones_dil: [],
    deseada: 10,
    volumenes: [100, 250, 500, 1000],
    acento: "#4ea6dc",
    sombra: "#4ea6dc44",
    icon: "⚗️",
  },
};

function calcular({ cConc, cDil, cDeseada, vFinal }) {
  const pConc = Math.abs(cDeseada - cDil);
  const pDil  = Math.abs(cConc - cDeseada);
  const pTot  = pConc + pDil;
  return {
    pConc, pDil, pTot,
    vConc: pTot === 0 ? 0 : (pConc / pTot) * vFinal,
    vDil:  pTot === 0 ? 0 : (pDil  / pTot) * vFinal,
  };
}

// ── State ──────────────────────────────────────────
let state = {
  tipo: "Salina",
  cConc: 20, cDil: 0.9, cDeseada: 3, vFinal: 500,
  labelConc: "Soletrol Na 20%", labelDil: "Sol. Salina 0.9%",
  selConc: PRESETS["Salina"].concentrada,
  installPrompt: null,
};

function getPreset() { return PRESETS[state.tipo]; }

function setState(patch) {
  state = { ...state, ...patch };
  render();
}

// ── DOM helpers ─────────────────────────────────────
function el(id) { return document.getElementById(id); }

function makeParticles() {
  const container = el("particles");
  container.innerHTML = "";
  for (let i = 0; i < 32; i++) {
    const d = document.createElement("div");
    d.className = "particle";
    d.style.cssText = `
      left:${(i*37+11)%100}%;
      top:${(i*53+7)%100}%;
      width:${(i%3)*0.9+0.5}px;
      height:${(i%3)*0.9+0.5}px;
      --dur:${((i%4)*1.2+1.8).toFixed(1)}s;
      --del:${((i%6)*0.6).toFixed(1)}s;
    `;
    container.appendChild(d);
  }
}

// ── Render ───────────────────────────────────────────
function render() {
  const app = el("app");

  // Guardar foco antes de redibujar → teclado no se cierra
  const focused = document.activeElement;
  const focusedId = focused?.dataset?.fieldId || null;
  const cursorPos = focused?.selectionStart ?? null;

  const preset = getPreset();
  const ac = preset.acento;
  const sh = preset.sombra;

  const valido = state.cConc > state.cDil && state.cDeseada > state.cDil && state.cDeseada < state.cConc;
  const res = valido ? calcular({ cConc: state.cConc, cDil: state.cDil, cDeseada: state.cDeseada, vFinal: state.vFinal }) : null;

  const amp = state.selConc?.ampolla || null;
  const numAmpollas = amp && res ? res.vConc / amp.volMl : null;
  const ampollasCeil = numAmpollas !== null ? Math.ceil(numAmpollas) : null;
  const sobranteMl = ampollasCeil !== null ? (ampollasCeil * amp.volMl) - res.vConc : 0;

  const pasos = res ? (() => {
    const s = [];
    if (amp && ampollasCeil !== null) {
      if (sobranteMl > 0.05) {
        s.push(`Tome ${ampollasCeil} ampolla${ampollasCeil>1?"s":""} de ${state.labelConc}.`);
        s.push(`Descarte ${sobranteMl.toFixed(1)} mL de esas ampollas. Lo que queda son los ${res.vConc.toFixed(1)} mL necesarios.`);
      } else {
        s.push(`Tome ${ampollasCeil} ampolla${ampollasCeil>1?"s":""} completa${ampollasCeil>1?"s":""} de ${state.labelConc} (${res.vConc.toFixed(1)} mL exactos).`);
      }
    } else {
      s.push(`Tome ${res.vConc.toFixed(1)} mL de ${state.labelConc}.`);
    }
    s.push(`Tome ${res.vDil.toFixed(1)} mL de ${state.labelDil}.`);
    s.push(`Mezcle ambos en el frasco. Obtendra ${state.vFinal} mL de solucion al ${state.cDeseada}%.`);
    return s;
  })() : null;

  app.innerHTML = `
    <!-- HEADER -->
    <div class="header">
      <div class="badge-row">
        <span class="badge">MEDISHORT360</span>
        <span class="badge">Diluciones IV</span>
        <span class="badge">Metodo de Aligacion</span>
      </div>

      <div class="logo-wrap">
        <div class="logo-ring-spin"></div>
        <div class="logo-ring-pulse"></div>
        <img src="${LOGO_B64}" alt="MEDISHORT360" class="logo-img"/>
      </div>

      <div class="app-title">Regla del Aspa</div>
      <div class="app-sub">Calculadora de Diluciones IV &mdash; Metodo de Aligacion</div>
      <div class="app-brand">📺 Canal MEDISHORT360-ASPA</div>
    </div>

    <!-- TABS -->
    <div class="tabs">
      ${Object.entries(PRESETS).map(([k,v]) => `
        <button class="tab-btn ${state.tipo===k?"active":""}"
          style="${state.tipo===k ? `background:${v.acento};box-shadow:0 0 12px ${v.sombra}` : ""}"
          onclick="handleTipo('${k}')">${v.icon} ${k}</button>
      `).join("")}
    </div>

    <!-- CONCENTRACIONES -->
    <div class="card" style="border-color:${ac}44;box-shadow:0 0 20px ${sh}">
      <div class="card-label">Presentaciones disponibles</div>
      ${preset.opciones_conc.length ? `
        <div class="pill-sub">Concentrada:</div>
        <div class="pill-row">${preset.opciones_conc.map(op=>`
          <button class="pill-btn" onclick="handleSelConc(${JSON.stringify(op).replace(/"/g,"'")})"
            style="border:1px solid ${state.cConc===op.value ? ac : "#1e2d50"};
                   background:${state.cConc===op.value ? ac+"33" : "transparent"};
                   color:${state.cConc===op.value ? ac : "#7a8aac"};
                   font-weight:${state.cConc===op.value ? "700" : "400"}">${op.label}</button>
        `).join("")}</div>
      ` : ""}
      ${preset.opciones_dil.length ? `
        <div class="pill-sub" style="margin-top:4px">Diluida:</div>
        <div class="pill-row" style="margin-bottom:14px">${preset.opciones_dil.map(op=>`
          <button class="pill-btn" onclick="handleSelDil(${JSON.stringify(op).replace(/"/g,"'")})"
            style="border:1px solid ${state.cDil===op.value ? ac : "#1e2d50"};
                   background:${state.cDil===op.value ? ac+"33" : "transparent"};
                   color:${state.cDil===op.value ? ac : "#7a8aac"};
                   font-weight:${state.cDil===op.value ? "700" : "400"}">${op.label}</button>
        `).join("")}</div>
      ` : ""}
      ${state.tipo==="Generica" ? `
        <div class="grid-2" style="margin-bottom:14px">
          <div class="input-wrap">
            <span class="input-label">Nombre conc.</span>
            <input class="text-input" value="${state.labelConc}" data-field-id="labelConc" oninput="setState({labelConc:this.value})"/>
          </div>
          <div class="input-wrap">
            <span class="input-label">Nombre dil.</span>
            <input class="text-input" value="${state.labelDil}" data-field-id="labelDil" oninput="setState({labelDil:this.value})"/>
          </div>
        </div>
      ` : ""}
      <div class="grid-2">
        ${numInput("labelConc", "state.labelConc", "cConc", state.labelConc, state.cConc, ac)}
        ${numInput("labelDil",  "state.labelDil",  "cDil",  state.labelDil,  state.cDil,  ac)}
      </div>
    </div>

    <!-- OBJETIVO -->
    <div class="card" style="border-color:${ac}44;box-shadow:0 0 20px ${sh}">
      <div class="card-label">Objetivo</div>
      <div class="grid-2">
        ${numInput("Concentracion deseada", null, "cDeseada", "Concentracion deseada", state.cDeseada, ac)}
        <div class="input-wrap">
          <span class="input-label">Volumen final</span>
          <div class="vol-row" style="margin-bottom:6px">
            ${preset.volumenes.map(v=>`
              <button class="vol-btn" onclick="setState({vFinal:${v}})"
                style="background:${state.vFinal===v ? ac : "#101830"};
                       color:${state.vFinal===v ? "#fff" : "#7a8aac"};
                       font-weight:${state.vFinal===v ? "700" : "400"};
                       box-shadow:${state.vFinal===v ? `0 0 8px ${sh}` : "none"}">${v}</button>
            `).join("")}
          </div>
          <div class="input-row" style="border-color:${ac}44">
            <input type="number" value="${state.vFinal}" min="1"
              data-field-id="vFinal"
              oninput="setState({vFinal:parseFloat(this.value)||0})"
              style="background:transparent;border:none;color:#f0f4ff;font-size:16px;
                     font-family:'DM Mono',monospace;padding:8px 12px;width:100%;outline:none"/>
            <span class="input-unit">mL</span>
          </div>
        </div>
      </div>
    </div>

    <!-- ERROR -->
    ${!valido ? `
      <div class="error-box">
        ⚠️ La concentracion deseada debe estar <strong>entre</strong>
        la diluida (${state.cDil}%) y la concentrada (${state.cConc}%).
      </div>
    ` : ""}

    <!-- RESULTADO -->
    ${res ? `
      <div class="result-card" style="border-color:${ac}55;box-shadow:0 0 30px ${sh}">
        <div class="result-header">
          <div class="card-label">Diagrama del Aspa</div>
          <div class="aspa-grid">
            <div class="aspa-cell">
              <div class="aspa-pct" style="color:${ac};text-shadow:0 0 10px ${sh}">${state.cConc}%</div>
              <div class="aspa-name">${state.labelConc}</div>
            </div>
            <div>
              <svg viewBox="0 0 60 60" width="60" height="60">
                <line x1="5" y1="5" x2="55" y2="55" stroke="#1e2d50" stroke-width="1.5"/>
                <line x1="55" y1="5" x2="5" y2="55" stroke="#1e2d50" stroke-width="1.5"/>
                <circle cx="30" cy="30" r="14" fill="#101830" stroke="${ac}" stroke-width="1.5"/>
                <text x="30" y="34" text-anchor="middle" fill="${ac}" font-size="10"
                  font-weight="700" font-family="monospace">${state.cDeseada}%</text>
              </svg>
            </div>
            <div class="aspa-cell">
              <div class="aspa-pct" style="color:#7a8aac">${state.cDil}%</div>
              <div class="aspa-name">${state.labelDil}</div>
            </div>
            <div class="aspa-cell">
              <div class="aspa-parts">→ partes</div>
              <div class="aspa-pval">${res.pConc.toFixed(2)}</div>
            </div>
            <div></div>
            <div class="aspa-cell">
              <div class="aspa-parts">→ partes</div>
              <div class="aspa-pval">${res.pDil.toFixed(2)}</div>
            </div>
          </div>
          <div class="aspa-total">
            Total: <span style="color:#f0f4ff;font-family:'DM Mono',monospace;font-weight:600">
              ${res.pTot.toFixed(2)} partes
            </span>
          </div>
        </div>

        <div class="result-body">
          <div class="card-label">Volumenes para ${state.vFinal} mL</div>
          <div class="vol-result-grid">
            ${[
              {label:state.labelConc, v:res.vConc, c:state.cConc, color:ac},
              {label:state.labelDil,  v:res.vDil,  c:state.cDil,  color:"#4ea6dc"},
            ].map(b=>`
              <div class="vol-block" style="border-color:${b.color};box-shadow:0 0 12px ${b.color}22">
                <div class="vol-block-label">${b.label} (${b.c}%)</div>
                <div class="vol-block-val" style="color:${b.color};text-shadow:0 0 8px ${b.color}66">
                  ${b.v.toFixed(1)} <span>mL</span>
                </div>
                <div class="vol-bar-bg">
                  <div class="vol-bar" style="width:${(b.v/state.vFinal*100).toFixed(1)}%;
                    background:linear-gradient(90deg,${b.color}88,${b.color})"></div>
                </div>
                <div class="vol-pct">${(b.v/state.vFinal*100).toFixed(1)}% del total</div>
              </div>
            `).join("")}
          </div>

          ${amp && ampollasCeil !== null ? `
            <div class="amp-block" style="background:${ac}15;border:1px solid ${ac}55">
              <span class="amp-icon">💉</span>
              <div>
                <div class="amp-title" style="color:${ac}">
                  ${ampollasCeil} ampolla${ampollasCeil>1?"s":""} de ${state.labelConc}
                </div>
                <div class="amp-sub">
                  Necesitas ${res.vConc.toFixed(1)} mL exactos.
                  ${sobranteMl > 0.05
                    ? ` Usa ${ampollasCeil} amp → descartar ${sobranteMl.toFixed(1)} mL antes de agregar.`
                    : " Usa exactamente esa cantidad."}
                </div>
              </div>
            </div>
          ` : ""}

          <div class="verif-row">
            <span class="verif-label">Verificacion</span>
            <span class="verif-val">
              ${res.vConc.toFixed(1)} + ${res.vDil.toFixed(1)} =
              <strong style="color:${ac}">${(res.vConc+res.vDil).toFixed(1)} mL</strong>
            </span>
          </div>
        </div>
      </div>

      <!-- PASOS -->
      <div class="steps-card" style="border-color:${ac};
        background:linear-gradient(135deg,#101830,#150020);
        box-shadow:0 0 30px ${sh}">
        <div class="steps-title" style="color:${ac}">
          <span>🩺</span> Como prepararlo — paso a paso
        </div>
        <div class="steps-list">
          ${pasos.map((p,i)=>`
            <div class="step-row">
              <div class="step-num" style="background:linear-gradient(135deg,${ac},#c41230);
                box-shadow:0 0 8px ${sh}">${i+1}</div>
              <p class="step-text">${p}</p>
            </div>
          `).join("")}
        </div>
      </div>
    ` : ""}

    <!-- FOOTER -->
    <div class="footer">
      <p>Solo para referencia clinica · Verificar con profesional de salud</p>
      <strong>🩺 MEDISHORT360 · Canal Medico Educativo</strong>
    </div>
  `;

  // Restaurar foco → teclado permanece abierto
  if (focusedId) {
    const target = document.querySelector('[data-field-id="' + focusedId + '"]');
    if (target) {
      target.focus();
      try { target.setSelectionRange(cursorPos, cursorPos); } catch(e) {}
    }
  }
}

// ── Helpers for number input HTML ────────────────────
function numInput(labelStr, _labelStateKey, stateKey, labelDisplay, value, ac) {
  return `
    <div class="input-wrap">
      <span class="input-label">${labelDisplay}</span>
      <div class="input-row" style="border-color:${ac}44">
        <input type="number" value="${value}" min="0" max="100" step="0.1"
          data-field-id="${stateKey}"
          oninput="setState({${stateKey}:parseFloat(this.value)||0})"
          style="background:transparent;border:none;color:#f0f4ff;font-size:18px;
                 font-family:'DM Mono',monospace;padding:10px 12px;width:100%;outline:none"/>
        <span class="input-unit" style="color:${ac}">%</span>
      </div>
    </div>
  `;
}

// ── Event handlers (global, called from inline HTML) ──
window.handleTipo = function(t) {
  const p = PRESETS[t];
  setState({
    tipo: t,
    selConc: p.concentrada,
    cConc: p.concentrada.value,
    cDil: p.diluida.value,
    cDeseada: p.deseada,
    vFinal: 500,
    labelConc: p.concentrada.label,
    labelDil: p.diluida.label,
  });
};

window.handleSelConc = function(op) {
  setState({ selConc: op, cConc: op.value, labelConc: op.label });
};

window.handleSelDil = function(op) {
  setState({ cDil: op.value, labelDil: op.label });
};

// ── Install PWA ───────────────────────────────────────
window.addEventListener("beforeinstallprompt", (e) => {
  e.preventDefault();
  state.installPrompt = e;
  el("install-btn").classList.add("visible");
});
window.addEventListener("appinstalled", () => {
  el("install-btn").classList.remove("visible");
  state.installPrompt = null;
});
window.handleInstall = function() {
  if (!state.installPrompt) return;
  state.installPrompt.prompt();
  state.installPrompt = null;
  el("install-btn").classList.remove("visible");
};

// ── Service Worker ────────────────────────────────────
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker.register("./sw.js")
      .then(() => console.log("SW registrado"))
      .catch(err => console.log("SW error:", err));
  });
}

// ── Init ──────────────────────────────────────────────
document.addEventListener("DOMContentLoaded", () => {
  makeParticles();
  render();
});
